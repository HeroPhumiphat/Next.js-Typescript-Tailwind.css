                                ...
                                <Link href={'/users/' + user.id} key={user.id}>
                                    <div className='text-center border mx-1 py-4 w-[270px] rounded-sm hover:cursor-pointer hover:scale-110 z-10 hover:z-20 bg-white transition delay-150 duration-300 ease-in-out' style={{boxShadow: '0 0 12px #ccc '}}>
                                        <div className='flex items-center justify-center'>
                                            <div className='relative w-20 h-20 rounded-full my-2' style={{backgroundImage: `url(${user.image})`, backgroundSize: 'cover', backgroundPosition: 'center'}}>
                                                {/* <FontAwesomeIcon icon={faUser} className='text-white text-2xl' /> */}
                                            </div>
                                        </div>
                                        <p className='text-neutral-400'>{user.firstName} {user.lastName}</p>
                                        <p>{user.maidenName}</p>
                                    </div>
                                </Link>
                            ))
